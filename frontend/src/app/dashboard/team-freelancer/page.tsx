...
import {
  fetchTeamFreelancerBidDays,
  fetchTeamFreelancerBids,
  type BidDay,
  type TeamBid,
} from "@/lib/bids";
...
export default function TeamFreelancerPage() {
  ...
  const loadDays = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        const rows = await fetchTeamFreelancerBidDays();
        ...
        if (!silent) {
          setError(
            err instanceof Error
              ? err.message
              : "Failed to load team freelancer bids",
          );
        }
      }
    },
  ...
  const loadDayBids = useCallback(
    async (dayKey: string, options?: { silent?: boolean }) => {
      ...
      try {
        const rows = await fetchTeamFreelancerBids({ date: dayKey });
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-slate-600">
          Freelancer bid URLs grouped by day, with one column per sub team.
        </p>
      </div>

      {loading ? (
        <p className="text-sm text-slate-500">Loading team freelancer bids…</p>
      ) : error ? (
